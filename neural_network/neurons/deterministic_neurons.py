...
####################################################################
class neuron():
	def __init__(self, pose, id_, log_history=False, decay_rate=False):
		# self.input_weights = []
		# self.input_ids = []
		...
		self.decay_rate = 1 # 0.025
		self.starting_transmition_value = 1
		self.pose = pose
		self.id = id_
	...
	def strengthen_conenction(self, output_id):
		self.output_transmition_values[self.output_ids.index(output_id)] += 1
